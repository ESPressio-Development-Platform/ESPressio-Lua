...
class SerialConsole {
    ...
    void finish() {
        if (overflow_) {
            Serial.println("ERROR: line too long; discarded without execution");
        } else if (used_ != 0) {
            try {
                const auto result = handler_.invoke({line_, used_});
                if (result) Serial.println("OK");
                else {
                    Serial.print("ERROR: ");
                ...
                Serial.println("ERROR: native exception");
            }
        }
        used_ = 0;
        overflow_ = false;
...
public:
    /// <summary>Borrows the application's local Lua console handler.</summary>
