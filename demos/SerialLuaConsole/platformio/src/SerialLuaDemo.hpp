 ...
 * Fixed members on the ESP32/Xtensa ILP32 reference ABI:
 * - led_ (Led): 8 bytes
 * - rgb_ (RgbLed): 8 bytes
 * - buzzer_ (Buzzer): 4 bytes
 * - print_ (LuaSerialPrint): 4 bytes
 * - ledType_/rgbType_/buzzerType_ (Lua::Type<T>): 8 bytes each
 * - script_ (Lua::Instance): 628 bytes
 * - command_ (LuaCommandHandler): 8 bytes
 * - console_ (SerialConsole): 1036 bytes
 * - ready_ (bool): 1 byte plus aggregate alignment
 * Approximate containing-object size: 1724 bytes on the reference ABI.
 * Lua type/module/VM storage remains bounded by ESPressio-Lua's configured memory/resource
 * contracts; the local console handler itself owns no dynamic storage.
 * Confidence: medium; compile-time sizeof on the concrete target remains authoritative.
...
class SerialLuaDemo {
    ...
    LuaCommandHandler command_{script_, print_};
    SerialConsole console_{command_};
    bool ready_ = false;
